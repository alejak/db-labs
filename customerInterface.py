...
class DBContext:
    ...
    def shipments(self):
        ...
        try:
            ID = int(raw_input("cutomerID: "))
        except(NameError,ValueError,TypeError,SyntaxError):
            print("That  was  not  a  number....  :(")
            return
        # These names inputs are terrible and allow injection attacks.
        #  So think that they can enter: Hilbert' OR 'a'='a
        fname=  str(pgdb.escape_string(raw_input("First Name: ").strip()))
        lname=  str(pgdb.escape_string(raw_input("Last Name: ").strip()))
        # THIS IS NOT RIGHT YOU MUST FIGURE OUT WHAT QUERY MAKES SENSE
        query ="""SELECT * FROM customers WHERE customer_id='%s' and first_name= '%s' and last_name= '%s';"""%(ID,fname,lname)
        print query
        ...
        query ="""SELECT shipment_id,ship_date,isbn,title FROM shipments inner join editions using(isbn) join books using(book_id) WHERE customer_id =%s;"""%(ID)
