...
# The Python DB-API allows the # The P# The Py# The Py# The Python DB-API allows the python code below to be used on any DB-APIthon DB-API allows the python code below to be used on any DB-APIthon DB-API allows the python code below to be used on any DB-APIython DB-API allows the python code below to be used on any DB-APIpython code below to be used on any DB-API
# we need to call the connect function with the right parameters some
# of wheich we 'hard code here such as the host, wnd others we# The Pyt# The Pyt# The Pyt# The Pyt# The Pyt# The Pyt# The Pyt# The Pyth# The Pyth# The Pyth# The Python DB-API allows the python code below to be used on any DB-APIon DB-API allows the python code below to be used on any DB-APIon DB-API allows the python code below to be used on any DB-APIon DB-API allows the python code below to be used on any DB-APIhon DB-API allows the python code below to be used on any DB-APIhon DB-API allows the python code below to be used on any DB-APIhon DB-API allows the python code below to be used on any DB-APIhon DB-API allows the python code below to be used on any DB-APIhon DB-API allows the python code below to be used on any DB-APIhon DB-API allows the python code below to be used on any DB-APIhon DB-API allows the python code below to be used on any DB-API call
# the built in python function raw_input to get from the user.  All are stored in a variable that we chose to call params.# The Python DB-API allows the python code below to be used on any DB-API
...
# here columns becomes the string that you type at prompt for Choose columns.
    ...
    def remove(self):
        ...
        try:
            removal = """ DELETE FROM %s %s; """%("".join(tables),"" if filters == "" else "WHERE %s"%filters)
        except (NameError,ValueError, TypeError,SyntaxError):
            print ("  Bad input.")
            return

        self.cur.execute(removal)
    ...
    def insert(self):
        ...
        values = raw_input("Specify values: ")

        print(values)
        try:
            insertion = """ INSERT into %s VALUES %s; """%("".join(tables), values)
        except (NameError,ValueError, TypeError,SyntaxError):
            print ("  Bad input.")
            return
